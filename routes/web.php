...
use App\Http\Controllers\Web\Auth\DotNetLoginController;
...
use App\Http\Controllers\Web\Admin\AdminCustomerAgreementController;
...
use App\Http\Controllers\Web\Customer\CustomerTaggedAgreementController;
...
Route::middleware('guest')->group(function () {
    ...
    // DotNet ERP Login (parallel path — does not affect the standard login)
    Route::get('/erp-login', [DotNetLoginController::class, 'showLogin'])->name('auth.dotnet.login');
    Route::post('/erp-login', [DotNetLoginController::class, 'login'])->name('auth.dotnet.login.post');
});
...
Route::prefix('admin')->middleware(['auth', 'admin.only'])->name('admin.')->group(function () {
    ...
    Route::get('/customers/{tenant}/agreements/workspace', [AdminCustomerAgreementController::class, 'workspace'])->name('tenants.agreements.workspace');
    Route::get('/customers/{tenant}/agreements/template/{agreement}', [AdminCustomerAgreementController::class, 'template'])->name('tenants.agreements.template');
    Route::post('/customers/{tenant}/agreements/draft', [AdminCustomerAgreementController::class, 'saveDraft'])->name('tenants.agreements.draft');
    Route::post('/customers/{tenant}/agreements/send', [AdminCustomerAgreementController::class, 'send'])->name('tenants.agreements.send');
    Route::get('/customers/{tenant}/agreements/signed-copies', [AdminCustomerAgreementController::class, 'signedCopies'])->name('tenants.agreements.signed');
    Route::get('/customers/{tenant}/agreements/signed-copies/{recordId}/download', [AdminCustomerAgreementController::class, 'downloadSignedCopy'])->name('tenants.agreements.signed.download');
...
Route::prefix('portal')->middleware(['auth', 'tenant.scope'])->name('customer.')->group(function () {
    ...
    Route::get('/contracts/tagged/{recordId}', [CustomerTaggedAgreementController::class, 'show'])->name('contracts.tagged.show');
    Route::get('/contracts/tagged/{recordId}/signature', [CustomerTaggedAgreementController::class, 'signature'])->name('contracts.tagged.signature');
    Route::post('/contracts/tagged/{recordId}/agree', [CustomerTaggedAgreementController::class, 'agree'])->name('contracts.tagged.agree');
